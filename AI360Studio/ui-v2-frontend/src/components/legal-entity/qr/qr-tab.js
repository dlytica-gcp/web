...
import qrLegalFetchData from "@/services/legal_qr_service";
import { convertDateToYMD } from "@/lib/utils/date_to_month";

const TabContent = (props) => {
  const { qr, legal_id } = props;
  ...
  const fetchAllData = async () => {
    try {
      ...
      const [derivedInfo, merchantInfo, subMerchantInfo, TerminalInfo] =
        await Promise.all([
          qrLegalFetchData.getindivid_qr_derived_info(legal_id),
          qrLegalFetchData.getindivid_qr_merchant_detail_info(legal_id),
          qrLegalFetchData.getindivid_qr_submerchant_detail_info(legal_id),
          qrLegalFetchData.getindivid_qr_terminal_detail_info(legal_id),
        ]);
  ...
  }, [legal_id,qrDerivedInfo,qrMerchantDetailsData,qrSubMerchantDetailsData,qrTerminalDetailsData]);
  return (
    <>
