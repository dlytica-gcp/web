  ...
  const fetchAllData = async () => {
    try {
      ...
      const [fundedloan, lciss, bgout, bgreg, lcout] = await Promise.all([
        legalLoanFetchData.getindivid_loan_details_info(legal_id),
        legalLoanFetchData.getnon_funded_loan_lc_issuance(legal_id),
        legalLoanFetchData.getnon_funded_bg_outstanding(legal_id),
        legalLoanFetchData.getnon_funded_bg_register(legal_id),
        legalLoanFetchData.getnon_funded_loan_lc_outstanding(legal_id),
      ]);
                        ...
                        {loanDataFunded &&
                          loanDataFunded?.map((item, index) => (
                            <tr key={index}>
                              <td>
                                <a
                                  href={`/legal.-entities/loan-account/funded-loan?foracid=${item["dim_loan_accounts_funded.foracid"]}`}
                                >
