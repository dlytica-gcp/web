...
const TabContent = (props) => {
  ...
  const fetchData = async (id) => {
    try {
      ...
      const result = await legalDepositFetchData.getdeposit_details(id);
                      ...
                      <tbody>
                        {depositDataSaving && depositDataSaving?.map((item, index) => (
                          <tr key={index}>
                            <td>
                              <Link
                                href={`/legal-entities/deposit-account/saving-deposit?foracid=${item["dim_gam.foracid"]}&cif_id=${item["dim_customers.cif_id"]}`}
                              >
                      ...
                      <tbody>
                        {depositDataFixed && depositDataFixed?.map((item, index) => (
                          <tr key={index}>
                            <td>
                              <Link
                                href={`/legal-entities/deposit-account/fixed-deposit?foracid=${item["dim_gam.foracid"]}&cif_id=${item["dim_customers.cif_id"]}`}
                              >
                                {item["dim_gam.foracid"]
                                  ? item["dim_gam.foracid"]
                                  : "No data"}
