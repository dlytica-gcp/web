...
const TabContent = (props) => {
  ...
  const fetchAllData = useCallback(async () => {
    try {
      ...
      const [derivedInfo, merchantInfo, TerminalInfo] = await Promise.all([
        posFetchData.getindivid_pos_derived_info(inv_id),
        posFetchData.getindivid_pos_merchant_detail_info(inv_id),
        posFetchData.getindivid_pos_terminal_detail_info(inv_id),
      ]);
  ...
  useEffect(() => {
    ...
    if (!hasFetched.current) {
      const dashboardElement = "pos-dashboard";
      const filters = {
        cif_id: inv_id,
      };
      const embed = async () => {
