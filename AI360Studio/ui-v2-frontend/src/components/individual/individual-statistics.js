  ...
  const fetchStatData = async (searchQueries, page = 1) => {
    ...
    try {
      const [result, totalResult] = await Promise.all([
        await individualCustomerFetchData.getStatistic({
          ...searchQueries,
          offset: (page - 1) * ITEMS_PER_PAGE,
          limit: ITEMS_PER_PAGE,
        }),
        await individualCustomerFetchData.getTotalCount(searchQueries),
      ]);
      ...
      <div className="container-fluid p-0">
        ...
        <div className="row mb-2 mb-xl-3">
          <div className="col-auto d-none d-sm-block">
            <h3>
              <strong>Individual</strong> Statistics
            </h3>
          </div>
        </div>
                            ...
                            {individuals &&
                              individuals.map((individual, index) => (
                                <tr key={index}>
                                  <td>
                                    <Link
                                      href={`/individuals/${individual["dim_customers.cif_id"]
                                        }`}
                                    >
                                  ...
                                  <td>{individual["dim_customers.gender"]}</td>
                                  <td>
